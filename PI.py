...
class data:
    ...
    def pull(self):
        ...
        data.close()
        self.cpu_temp = raw_cputemp / 1000

        # CPU usage
        self.cpu_usage = float(os.popen("top -n1 | awk '/Cpu\(s\):/ {print $2}'").readline().strip())

        # RAM usage
        self.ram_usage = round(float(os.popen("free | awk '/Mem/ {print $3/$2}'").readline().strip()) * 100, 1)
